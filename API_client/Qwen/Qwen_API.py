"""
Table Reference Verification Pipeline
---------------------------------------
Compares cited tables in academic papers against original source tables
using multimodal LLMs (DashScope/Qwen). Images are uploaded to Alibaba Cloud OSS
for analysis, and results are saved in batches for fault tolerance.
"""
...
- **提取图片中与作者 "{author}" 相关的文字、图表或数据**（如作者提出的方法、其模型名称、对应结果等）。如果没有相关内容，则返回空字符串。
...
- 数据集缩写的全称（如 "ETD: End-to-End Turn-taking Dataset"）
- 指标缩写的含义（如 "SU: Speech Utterance", "IoU: Intersection over Union"）
- 符号说明（如 "↑ 表示性能提升", "† 表示使用额外数据"）
- 实验设置说明（如 "所有模型在相同硬件上测试"）
