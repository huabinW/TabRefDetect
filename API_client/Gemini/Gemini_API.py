...
SYSTEM_PROMPTS = {
    "table_parser": "You are an academic table parser. Your task is to analyze the provided table/image.",
    "caption_parser": "You are an academic table caption and context parser. Your task is to extract explanatory information from the provided image.",
    "semantic_reviewer": "You are a rigorous academic reviewer. Your task is to determine whether the following two pieces of content are semantically matched."
}
...
def analyze_image(image_path, author=None, mode="citekey", paper_id=None):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if mode == "citekey":
                prompt = f"""Please analyze the image content in detail, paying special attention to the following:
1. Extract ALL numerical values (e.g., Accuracy, F1-score, BLEU, etc.). Clearly distinguish between decimals (e.g., 0.85) and percentages (e.g., 85%). Do not omit any values.
2. Identify all model names (e.g., BERT, ResNet, Ours, Proposed Method), dataset names (e.g., ImageNet, COCO, SQuAD), and evaluation metrics (e.g., Accuracy, ROUGE-L, mAP).
3. If it is a table, extract key data row by row, especially model comparison results.
4. Do not extract content related to specific authors (as this is the cited paper's table).
5. The description should be as complete and accurate as possible, without omitting any key data.
Return ONLY the following JSON format:
{{"description": "Detailed description containing all key data, models, metrics, and datasets"}}"""
            else:
                prompt = f"""Please analyze the image content and extract the following information:
1. Describe the overall content, including key information from text, charts, or tables. Pay special attention to whether numerical values are decimals or percentages. Do not omit specific values.
2. Extract text, charts, or data related to author "{author}" (e.g., their proposed methods, model names, corresponding results). If none, return an empty string.
3. If the image contains a table, extract key data and explicitly mark its association with author "{author}".
4. Focus on model names, datasets, evaluation metrics, and other key information.
Example (for format reference only, do not copy content):
"AdaptCap (Wang et al., 2023) on COCO-Caption: BLEU-4=38.5, METEOR=28.9, CIDEr=124.7"
Return ONLY the following JSON format:
{{"description": "Overall description of the image", "author_related_content": "Content related to author '{author}' (empty string if none)"}}"""
            ...
            messages = [
                {"role": "system", "content": SYSTEM_PROMPTS["table_parser"]},
                {
                    "role": "user",
                    "content": [
...
- Content A (Cited Paper): "{claim}"
- Content B (Content related to author "{author}" in the citing paper): "{author_content}"
...
3. If Content A fully covers the author-related data in Content B (consistent values, similar models/datasets) → match successful, score = 1.0.
4. If Content A partially misses author-related data (e.g., missing a metric or value) → score between 0.1–0.9, penalized proportionally.
5. If Content A completely omits author-related content → score = 0.0.
6. Ignore parts of Content B unrelated to the author.

【Output Requirements】
Return ONLY a strict JSON object containing:
"score": A float between 0.0 and 1.0
"explanation": A concise reason for matching/mismatching, pointing out missing or consistent key points
Do not include any other text."""

            messages = [
                {"role": "system", "content": SYSTEM_PROMPTS["semantic_reviewer"]},
    ...
    try:
        ...
        for group in tab_groups:
            ...
            for cap_img in caption_images:
                ...
                caption_prompt = f"""You are viewing a table caption. Extract ONLY explanatory information about the table content, such as:
1. Full names of dataset abbreviations (e.g., "ETD: End-to-End Turn-taking Dataset")
2. Meanings of metric abbreviations (e.g., "SU: Speech Utterance", "IoU: Intersection over Union")
3. Symbol explanations (e.g., "↑ indicates performance improvement", "† indicates extra data used")
4. Experimental setup notes (e.g., "All models tested on identical hardware")
DO NOT extract author conclusions, method comparisons, or subjective evaluations (e.g., "our method is better").
Organize the extracted information into a single coherent supplementary sentence.
Return ONLY the following JSON format:
{{"table_annotations": "Supplementary explanation (single sentence)"}}"""
                messages = [
                    {"role": "system", "content": SYSTEM_PROMPTS["caption_parser"]},
                    {
                        "role": "user",
                        "content": [
