...
"""
            ...
            if attempt == max_retries - 1:
                log_error(paper_id, "LLMCallFailed", f"调用 GLM-4V 失败: {str(e)}",
                         traceback_msg=traceback.format_exc(),
...
batch_counter = 1
while True:
    batch_file = f"./glm-4.5v/glm-4v-plus_batch_{batch_counter}.json"
...
for i, entry in enumerate(data[last_counter:], start=last_counter):
    try:
        ...
        if counter % batch_size == 0:
            output_file_batch = f"./glm-4.5v/glm-4v-plus_batch_{counter // batch_size}.json"
...
# 最终保存
output_file = "./glm-4.5v/glm-4v-plus.json"
